...
export class CSVExporter {
  ...
  static generateCSV(data: ParsedFinancialStatements): string {
    ...
    const statementTypes = ["income_statement", "balance_sheet", "cash_flow"] as const;
    ...
    for (const statementType of statementTypes) {
      ...
      for (const year of years) {
        if (statementData[year]) {
          Object.keys(statementData[year]).forEach(item => allItems.add(item));
        }
      }
  ...
  static generateComparativeCSV(companies: string[]): string {
      ...
      for (const item of Array.from(allItems).sort()) {
        for (const year of sortedYears) {
          ...
          for (const company of companies) {
            ...
            const value = data?.statements[statementType][year]?.[item] || "N/A";
    ...
    // Create headers
    const headers = ["Statement", "Item", "Year", ...companies.map(c => this.formatCompanyName(c))];
