...
async function testCSVFunctionality() {
  ...
  try {
    ...
    console.log("\n📋 Test 1: List Available Companies");
    const companiesResponse = await axios.get(`${API_BASE_URL}/api/csv/companies`);
    console.log(`✅ Available companies: ${companiesResponse.data.companies.join(", ")}`);
    console.log(`📊 Total companies: ${companiesResponse.data.total}`);
    
    const companies = companiesResponse.data.companies;
    if (companies.length === 0) {
      console.log("⚠️  No companies available for CSV export. Run parsing first.");
      return;
    }

    // Test 2: Preview CSV for first company
    console.log(`\n👀 Test 2: Preview CSV for ${companies[0]}`);
    const previewResponse = await axios.get(`${API_BASE_URL}/api/csv/preview/${companies[0]}?limit=5`);
    console.log(`✅ Preview generated successfully`);
    console.log(`📊 Headers: ${previewResponse.data.preview.headers.join(", ")}`);
    ...
    console.log("\n📋 Sample data:");
    previewResponse.data.preview.data.slice(0, 3).forEach((row: any, index: number) => {
    ...
    if (companies.length > 1) {
      ...
      const compareCompanies = companies.slice(0, Math.min(3, companies.length));
      ...
      console.log(`📏 Content length: ${compareResponse.data.length} characters`);
      
      // Save comparative CSV
      const compareFile = path.join(csvDir, `comparison_${compareCompanies.join("_")}_test.csv`);
    ...
    if (error.response) {
      ...
      console.error("🔍 Check if the backend server is running on", API_BASE_URL);
    } else {
